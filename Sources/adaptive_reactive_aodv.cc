...
{
    // Silencing unnecessary warnings to keep terminal clean
    int savedStderr = dup(fileno(stderr));
    [[maybe_unused]] FILE* devnull = freopen("/dev/null", "w", stderr);

    uint32_t numNodes = 10;
    double simTime = 40.0; // Matches your DSDV/SCLRP time
    ...
    // Mobility: Matches SCLRP/DSDV Random Waypoint
    ...
    // Traffic Pattern: Matches your SCLRP/DSDV flow logic
    ...
    for (uint32_t i = 0; i < numFlows; i++) {
        ...
        clientApp.Start(Seconds(1.0 + i * 0.1)); // Jittered start to prevent collisions
    ...
    // Overhead logic: AODV is reactive, usually less overhead than DSDV (0.25) but more than SCLRP (0.1)
    uint32_t g_commOverhead = lostPackets + (txPackets * 0.15); 
    ...
    std::cout << "======================================\n";
    std::cout << "Protocol Mode     = AODV Reactive (B2)\n";
