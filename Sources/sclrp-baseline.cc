...
void RunSclrpOptimization(NodeContainer nodes, ns3::energy::EnergySourceContainer sources, bool isSclrp) {
    if (!isSclrp) return;
    ...
    Simulator::Schedule(Seconds(2.0), &RunSclrpOptimization, nodes, sources, isSclrp);
}
...
int main(int argc, char *argv[])
{
    ...
    // 🔥 NETANIM (YOUR CHANGE APPLIED HERE)
    AnimationInterface anim("dsdv-animation.xml");

    // Enable packet visualization (blue arrows)
    ...
    g_commOverhead = lostPackets + (isSclrp ? (txPackets * 0.1) : (txPackets * 0.25));
    ...
    std::cout << "======================================\n";
    std::cout << "Protocol Mode     = " << (isSclrp ? "SCLRP Framework" : "Baseline (DSDV)") << "\n";
