...
int main(int argc, char *argv[])
{
    ...
    for (uint32_t i = 0; i < numFlows; i++) {
        ...
        // 🔥 Swapped UDP Client for TCP OnOff Application
        OnOffHelper client("ns3::TcpSocketFactory", Address(InetSocketAddress(interfaces.GetAddress(dst), port)));
        client.SetAttribute("DataRate", StringValue("500kbps")); // Mimics high-density load
        client.SetAttribute("PacketSize", UintegerValue(packetSize));
        client.SetAttribute("MaxBytes", UintegerValue(totalPackets * packetSize));
    ...
    AnimationInterface anim("reno_congestion_control_anim.xml");
    ...
    // TCP generates ACKs, increasing the communicational overhead inherently
    g_commOverhead = lostPackets + (txPackets * 0.40); 
    ...
    std::cout << "======================================\n";
    std::cout << "Protocol Mode     = TCP Reno (Paper B7 Benchmark)\n";
    std::cout << "Number of Nodes   = " << numNodes << "\n";
    std::cout << "Target Packets    = " << totalPackets << " per flow\n";
